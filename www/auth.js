...
function initSupabase() { if (typeof window === "undefined") return;
...
// ── Register ──
function doRegister() { if (!supabase) return;
...
// ── Login ──
function doLogin() { if (!supabase) return;
  var email = document.getElementById("auth-email").value.trim();
  var pw = document.getElementById("auth-pw").value.trim();
...
// ── Logout ──
function doLogout() { if (!supabase) return;
